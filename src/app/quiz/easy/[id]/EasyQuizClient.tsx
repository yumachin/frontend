...
export default function EasyQuizClient({ question, id } : { question: QuestionType, id: string }) {
  const { user } = useUser()
  ...
  useEffect(() => {
    if (!user) return;
    router.push(`/quiz/easy/${user.stats.easyCorrectNum + 1}`)
  }, [user])
  ...
  const handleAnswer = async () => {
    try {
      ...
      if (res.message) {
        router.push(`/answer/${user.stats.easyCorrectNum+1}?level=easy&selectedAnswer=${selectedAnswer}`)
      } else {
